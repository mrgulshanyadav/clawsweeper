...
- The edit pass should rebase onto latest ${baseBranch}, address PR comments and review findings, fix CI/check failures, preserve release-note context when required, run the relevant validation, and keep iterating until the branch is ready or an external blocker is proven.
...
- Start by reproducing the bug or adding a failing regression test. If the bug
  cannot be reproduced on latest ${baseBranch}, report the blocker instead of opening a
  PR.
...
still valid on latest \`${repo}@${baseBranch}\`, inspect nearby code, and make the
